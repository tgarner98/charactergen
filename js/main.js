// Races and Names //

// Male Names //
var male = new Array();
male[0] = new Array( // Aarakocra //
    "Rek",
    "Khil",
    "Crilag",
    "Zed",
    "Qhed",
    "Rhiss",
    "Keck",
    "Ahk",
    "Kharrk",
    "Dirk"
);
male[1] = new Array( // Dragonborn //
    "Gompok",
    "Krakath",
    "Naqull",
    "Uthtir",
    "Certhar",
    "Drastis",
    "Kildar",
    "Myulmin",
    "Tazbroth",
    "Balzire"
);
male[2] = new Array( // Drow //
    "Yazlan",
    "Jivrak",
    "Itzar",
    "Nyrmil",
    "Vielin",
    "Nador",
    "Melakiir",
    "Nalfein",
    "Kodar",
    "Selakiir"
);
male[3] = new Array( // Dwarf //
    "Grahldor",
    "Tornik",
    "Bertharn",
    "Harmiir",
    "Balmyr",
    "Daermek",
    "Dalgron",
    "Therdor",
    "Banrik",
    "Galron"
);
male[4] = new Array( // Eldarin //
    "Jathas",
    "Panthas",
    "Yanmil",
    "Dageon",
    "Zoremin",
    "Vicrian",
    "Uridan",
    "Corbaver",
    "Brantorin",
    "Tavkul"
);
male[5] = new Array( // Elf //
    "Zinven",
    "Herfir",
    "Sylneiros",
    "Yinro",
    "Iliris",
    "Naelar",
    "Luzeiros",
    "Genmaris",
    "Virris",
    "Thedan"    
);
male[6] = new Array( // Gnome //
    "Zanlen",
    "Bilhim",
    "Raszu",
    "Nesston",
    "Tanvyn",
    "Calkas",
    "Keltor",
    "Farver",
    "Wilkas",
    "Ronji"    
);
male[7] = new Array( // Half-Elf //
    "Wilorin",
    "Belvalor",
    "Vanelor",
    "Osovar",
    "Yenros",
    "Tyrkoris",
    "Nilhorn",
    "Raffyr",
    "Vanorin",
    "Zylorin"    
);
male[8] = new Array( // Halfling //
    "Merlos",
    "Davmin",
    "Arlos",
    "Elzu",
    "Horzu",
    "Korzin",
    "Kasdak",
    "Zalrich",
    "Sharbin",
    "Pimdal"    
);
male[9] = new Array( // Human //
    "Rashin",
    "Fusim",
    "Rirden",
    "Shordar",
    "Garth",
    "Alun",
    "Zerder",
    "Hivif",
    "Dakren",
    "Kordim"    
);
male[10] = new Array( // Lizardfolk //
    "Tiltotu",
    "Kolrur",
    "Leslurk",
    "Girisk",
    "Dardor",
    "Nurko",
    "Sirkur",
    "Nech",
    "Jhilit",
    "Lorkit"    
);
male[11] = new Array( // Orc //
    "Bharzug",
    "Zhobar",
    "Ghok",
    "Ghog",
    "Nubron",
    "Lanakk",
    "Grubzol",
    "Ghavol",
    "Rabzokk",
    "Zhag"    
);
male[12] = new Array( // Tabaxi //
    "Hide",
    "Fang",
    "Quill",
    "Dew",
    "Scratch",
    "Lock",
    "Rock",
    "Stitch",
    "Gale",
    "Robin"   
);
male[13] = new Array( // Tiefling //
    "Skavius",
    "Valrus",
    "Mavilius",
    "Shathos",
    "Xarrus",
    "Malus",
    "Skamos",
    "Salreus",
    "Zarmir",
    "Revir"    
);



// Female Names //

var female = new Array();
female[0] = new Array( // Aarakocra //
    "Kha",
    "Rheca",
    "Arra",
    "Zuki",
    "Rhe",
    "Girri",
    "Daeqa",
    "Arri",
    "Su",
    "Ruie"
);
female[1] = new Array( // Dragonborn //
    "Thameila",
    "Abis",
    "Dister",
    "Pralkun",
    "Vyurinn",
    "Faewyn",
    "Zarinn",
    "Tuaxis",
    "Firuuc",
    "Irnoc"
);
female[2] = new Array( // Drow //
    "Quarri",
    "Faetyn",
    "Lyra",
    "Ilmra",
    "Yasala",
    "Zendaza",
    "Felynnia",
    "Halxae",
    "Vierva",
    "Ereldra"
);
female[3] = new Array( // Dwarf //
    "Nalvia",
    "Mystin",
    "Gymryn",
    "Karnis",
    "Runvia",
    "Edma",
    "Lyssri",
    "Edlyn",
    "Gwennis",
    "Lesgiel"
);
female[4] = new Array( // ELdarin //
    "Qixis",
    "Faehyssa",
    "Faegrys",
    "Savyre",
    "Velameiv",
    "Magqis",
    "Alcena",
    "Sameiv",
    "Ulvara",
    "Birxana"
);
female[5] = new Array( // Elf //
    "Sylthyra",
    "Inara",
    "Reydi",
    "Valdove",
    "Sylkrana",
    "Sylleth",
    "Bira",
    "Shadi",
    "Syldi",
    "Dayra"
);
female[6] = new Array( // Gnome //
    "Faenoa",
    "Fentra",
    "Eiliza",
    "Nysys",
    "Talys",
    "Zintra",
    "Jobi",
    "Tabi",
    "Nihani",
    "Nynove"
);
female[7] = new Array( // Half-Elf //
    "Sylphira",
    "Jenys",
    "Resnoa",
    "Zelstine",
    "Kyszira",
    "Reldiane",
    "Alystine",
    "Qidove",
    "Elgwynn",
    "Faeona"    
);
female[8] = new Array( // Halfling //
    "Hala",
    "Nedalyn",
    "Paezira",
    "Kithna",
    "Froyra",
    "Verni",
    "Fayni",
    "Diari",
    "Vawyn",
    "Nevira"    
);
female[9] = new Array( // Human //
    "Nalni",
    "Nihne",
    "Recha",
    "Tsui",
    "Evri",
    "Lithra",
    "Kefla",
    "Miao",
    "Cephani",
    "Kari"    
);
female[10] = new Array( // Lizardfolk //
    "Vegy",
    "Atega",
    "Rerdy",
    "Lipekri",
    "Ari",
    "Ishe",
    "Nagge",
    "Tri",
    "Va",
    "Thirga"    
);
female[11] = new Array( // Orc //
    "Kuansu",
    "Ghulla",
    "Bounsaa",
    "Migvo",
    "Illi",
    "Uwna",
    "Showni",
    "Edgi",
    "Ghanza",
    "Bedduk"    
);
female[12] = new Array( // Tabaxi //
    "Sapphire",
    "Bubble",
    "Dream",
    "Blossom",
    "Misty",
    "Night",
    "Summer",
    "River",
    "Angel",
    "Sky"
);
female[13] = new Array( // Tiefling //
    "Belki",
    "Livari",
    "Yuhiri",
    "Griyola",
    "Seiriza",
    "Riki",
    "Satari",
    "Nayis",
    "Roza",
    "Nithari",
    "Phoebe"
);

var charclass = new Array(
    "Barbarian",
    "Bard",
    "Cleric",
    "Druid",
    "Fighter",
    "Monk",
    "Paladin",
    "Ranger",
    "Rogue",
    "Sorcerer",
    "Warlock",
    "Wizard"
);




// Character Generator //
function genChar() {
    var gender = document.getElementsByName("gender");
    for (i = 0; i < gender.length; i++){ // Name Generator //
        if (gender[i].checked){
            document.getElementById("givenName").innerHTML = gender[i].value;
            var race = document.getElementById("race").value;
            if (gender[0].checked){
                var ran = Math.floor(Math.random() * ((male[race].length)-0) + 0); // Generates number between 0 and length of array to randomly select name. Can add names freely instead of editing max number.
                document.getElementById("givenName").innerHTML = male[race][ran]; // Writes selected name to HTML
            } else {
                var ran = Math.floor(Math.random() * ((female[race].length)-0) + 0);
                document.getElementById("givenName").innerHTML = female[race][ran];
            };
        };
    };
    var stat = [
        str,
        dex,
        int,
        wis,
        cha,
        con
    ];
    var str;
    var dex;
    var int;
    var wis;
    var cha;
    var con;

    var charClass = document.getElementById("charClass").value;
    var level = document.getElementById("charLevel").value;
    var rolls = [ // Makes six rolls between 10 and 18 //
        Math.floor(Math.random() * (19-10) + 10),
        Math.floor(Math.random() * (19-10) + 10),
        Math.floor(Math.random() * (19-10) + 10),
        Math.floor(Math.random() * (19-10) + 10),
        Math.floor(Math.random() * (19-10) + 10),
        Math.floor(Math.random() * (19-10) + 10) 
    ];

    // Stat Generator //

    rolls.sort();

    // Racial Bonuses //

    
    // Distributes rolls array values to stat variables based on selected character class

    // Barbarian //
    if (charClass == 0) {
        stat[0] = rolls[5];
        stat[5] = rolls[4];
        stat[1] = rolls[3];
        stat[4] = rolls[2];
        stat[3] = rolls[1];
        stat[2] = rolls[0];


        // Bard //
    } else if (charClass == 1) {
        stat[4] = rolls[5];
        stat[1] = rolls[4];
        stat[2] = rolls[3];
        stat[3] = rolls[2];
        stat[5] = rolls[1];
        stat[0] = rolls[0];

        // Cleric //
    } else if (charClass == 2) {
        stat[3] = rolls[5];
        stat[4] = rolls[4];
        stat[2] = rolls[3];
        stat[5] = rolls[2];
        stat[1] = rolls[1];
        stat[0] = rolls[0];

        // Druid //
    } else if (charClass == 3) {
        stat[3] = rolls[5];
        stat[2] = rolls[4];
        stat[4] = rolls[3];
        stat[5] = rolls[2];
        stat[1] = rolls[1];
        stat[0] = rolls[0];

        // Fighter //
    } else if (charClass == 4) {
        stat[0] = rolls[5];
        stat[1] = rolls[4];
        stat[5] = rolls[3];
        stat[4] = rolls[2];
        stat[3] = rolls[1];
        stat[2] = rolls[0];

        // Monk //
    } else if (charClass == 5) {
        stat[1] = rolls[5];
        stat[3] = rolls[4];
        stat[2] = rolls[3];
        stat[4] = rolls[2];
        stat[0] = rolls[1];
        stat[5] = rolls[0];

        // Paladin //
    } else if (charClass == 6) {
        stat[0] = rolls[5];
        stat[4] = rolls[4];
        stat[5] = rolls[3];
        stat[1] = rolls[2];
        stat[3] = rolls[1];
        stat[2] = rolls[0];

        // Ranger //
    } else if (charClass == 7) {
        stat[1] = rolls[5];
        stat[3] = rolls[4];
        stat[2] = rolls[3];
        stat[4] = rolls[2];
        stat[5] = rolls[1];
        stat[0] = rolls[0];

        // Rogue //
    } else if (charClass == 8) {
        stat[1] = rolls[5];
        stat[2] = rolls[4];
        stat[4] = rolls[3];
        stat[3] = rolls[2];
        stat[0] = rolls[1];
        stat[5] = rolls[0];

        // Sorcerer //
    } else if (charClass == 9) {
        stat[4] = rolls[5];
        stat[5] = rolls[4];
        stat[2] = rolls[3];
        stat[3] = rolls[2];
        stat[1] = rolls[1];
        stat[0] = rolls[0];

        // Warlock //
    } else if (charClass == 10) {
        stat[4] = rolls[5];
        stat[3] = rolls[4];
        stat[2] = rolls[3];
        stat[5] = rolls[2];
        stat[1] = rolls[1];
        stat[0] = rolls[0];

        // Wizard //
    } else {
        stat[2] = rolls[5];
        stat[3] = rolls[4];
        stat[5] = rolls[3];
        stat[4] = rolls[2];
        stat[1] = rolls[1];
        stat[0] = rolls[0];
    };

    if (race = "0") { // Aarakocra Bonus: Dex
        stat[1] += 2;
    } else if (race = "1") { // Dragonborn Bonus: Str, Cha
        stat[0] += 2;
        stat[4] += 1;
    } else if (race = "2") { // Drow Bonus: Cha
        stat[4] += 1;
    } else if (race="3") { // Dwarf Bonus: Con
        stat[5] += 2;
    } else if (race="4") { // Eldarin Bonus: Int
        stat[2] += 1;
    } else if (race="5") { // Elf Bonus: Dex
        stat[1] += 2;
    } else if (race="6") { // Gnome Bonus: Int
        stat[2] += 2;
    } else if (race="7") { // Half-Elf Bonus: Cha
        stat[4] += 2;
    } else if (race="8") { // Halfling Bonus: Dex
        stat[1] += 2;
    } else if (race="9") { // Human Bonus: Everything
        stat[0] += 1;
        stat[1] += 1;
        stat[2] += 1;
        stat[3] += 1;
        stat[4] += 1;
        stat[5] += 1;
    } else if (race="10") { // Lizardfolk Bonus: Con, Wis
        stat[5] += 2;
        stat[3] += 1;
    } else if (race="11") { // Orc Bonus: Str, Con, -Int
        stat[0] += 2;
        stat[5] += 1;
        stat[2] -= 2;
    } else if (race="12") { // Tabaxi Bonus: Dex, Cha
        stat[1] += 2;
        stat[4] += 1;
    } else {                // Tiefling Bonus: Cha, Int
        stat[4] += 2;
        stat[2] += 1;
    };

    // Write final stat calculations to DOM
    document.getElementById("name-stats").innerText = "Name & Stats";
    document.getElementById("givenStrength").innerText = "Strength: " + stat[0];
    document.getElementById("givenDexterity").innerText = "Dexterity: " + stat[1];
    document.getElementById("givenConstitution").innerText = "Constitution: " + stat[5];
    document.getElementById("givenIntelligence").innerText = "Intelligence: " + stat[2];
    document.getElementById("givenWisdom").innerText = "Wisdom: " + stat[3];
    document.getElementById("givenCharisma").innerText = "Charisma: " + stat[4];

    // Skills //
    var skillMods = [
        0,
        0,
        0,
        0,
        0,
        0
    ];
    for (i=0; i < stat.length; i++) {
        if (stat[i] == 10 || stat[i] == 11) {
            skillMods[i] = 0;
        } else if (stat[i] == 12 || stat[i] == 13) {
            skillMods[i] = 1;
        } else if (stat[i] == 14 || stat[i] == 15) {
            skillMods[i] = 2;
        } else if (stat[i] == 16 || stat[i] == 17) {
            skillMods[i] = 3;
        } else if (stat[i] == 18 || stat[i] == 19) {
            skillMods[i] = 4;
        } else {
            skillMods[i] = 5;
        };
    };
    
    var givenAcrobatics = document.getElementById("givenAcrobatics");
    var givenAnimalHandling = document.getElementById("givenAnimalHandling");
    var givenArcana = document.getElementById("givenArcana");
    var givenAthletics = document.getElementById("givenAthletics");
    var givenDeception = document.getElementById("givenDeception");
    var givenHistory = document.getElementById("givenHistory");
    var givenInsight = document.getElementById("givenInsight");
    var givenIntimidation = document.getElementById("givenIntimidation");
    var givenInvestigation = document.getElementById("givenInvestigation");
    var givenMedicine = document.getElementById("givenMedicine");
    var givenNature = document.getElementById("givenNature");
    var givenPerception = document.getElementById("givenPerception");
    var givenPerformance = document.getElementById("givenPerformance");
    var givenPersuasion = document.getElementById("givenPersuasion");
    var givenReligion = document.getElementById("givenReligion");
    var givenSleightOfHand = document.getElementById("givenSleightOfHand");
    var givenStealth = document.getElementById("givenStealth");
    var givenSurvival = document.getElementById("givenSurvival");

    document.getElementById("your-skills").innerText = "Skills";
    givenAcrobatics.innerHTML = "Acrobatics: +" + skillMods[1];
    givenAnimalHandling.innerHTML = "Animal Handling: +" + skillMods[3];
    givenArcana.innerHTML = "Arcana: +" + skillMods[2];
    givenAthletics.innerHTML = "Athletics: +" + skillMods[0];
    givenDeception.innerHTML = "Deception: +" + skillMods[5];
    givenHistory.innerHTML = "History: +" + skillMods[2];
    givenInsight.innerHTML = "Insight: +" + skillMods[3];
    givenIntimidation.innerHTML = "Intimidation: +" + skillMods[5];
    givenInvestigation.innerHTML = "Investigation: +" + skillMods[2];
    givenMedicine.innerHTML = "Medicine: +" + skillMods[3];
    givenNature.innerHTML = "Nature: +" + skillMods[2];
    givenPerception.innerHTML = "Perception: +" + skillMods[3];
    givenPerformance.innerHTML = "Performance: +" + skillMods[5];
    givenPersuasion.innerHTML = "Persuasion: +" + skillMods[5];
    givenReligion.innerHTML = "Religion: +" + skillMods[2];
    givenSleightOfHand.innerHTML = "Sleight of Hand: +" + skillMods[1];
    givenStealth.innerHTML = "Stealth: +" + skillMods[1];
    givenSurvival.innerHTML = "Survival: +" + skillMods[3];

};